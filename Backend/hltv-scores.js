/**
 * Suivi des scores par map CS2 via hltv-match-api (auto-hébergé) :
 * https://github.com/fanden/hltv-match-api
 *
 ...
 * — il n'existe PAS d'endpoint "détail d'un match déjà terminé" comme
 * vlr.gg/v2/match/details?match_id=X. Sur un match live, elle renvoie :
 ...
 *
 * Stratégie adoptée ici : sonder /api/matches/live à intervalle régulier
 * PENDANT que le match est en direct, et figer le score d'une map au moment
 * précis où mapWins augmente — la map qui vient de se terminer avait le
 * score vu au sondage PRÉCÉDENT, juste avant l'incrémentation (au sondage
 * suivant, le compteur est déjà reparti à 0 sur la map suivante). Le tout
 * est gardé en mémoire (ce module) puis remonté à cs2-routes.js dès qu'un
 * match disparaît de la liste "running" PandaScore (= terminé), voir
 * `finalizeAndGet`.
 *
...
// l'instance auto-hébergée (single-service, pas de cache géant nécessaire).
 ...
 * tableau complet au même format que vlr-scores.js/PandaScore ({score1,
 * score2} par map, sans nom de map — HLTV ne l'expose pas sur ces
 * endpoints, contrairement à vlr.gg).
 *
